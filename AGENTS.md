...
- `app/any2api/web-assets` 是由 Vite 生成并提交的内嵌产物，禁止手工编辑；前端变更和完整本地打包统一使用 `cargo xtask package` 自动构建、同步、复核并生成 release 二进制。`pnpm build:embedded` 与 `pnpm check:embedded` 只保留为编排使用的低层原语。
