...
CREATE TABLE provider_endpoints_images (
    id TEXT PRIMARY KEY,
...
INSERT INTO provider_endpoints_images (
...
CREATE TABLE provider_credentials_images (
    id TEXT PRIMARY KEY,
    provider_endpoint_id TEXT NOT NULL
        REFERENCES provider_endpoints_images(id) ON DELETE RESTRICT,
...
INSERT INTO provider_credentials_images (
    ...
    secret_tail, proxy_profile_id, requests_per_minute, enabled, created_at, updated_at
)
SELECT
    id, provider_endpoint_id, label, label_key, credential_kind, secret_schema_version,
    secret_version, credential_generation, config_version, envelope_version, key_id,
...
CREATE TABLE provider_credential_models_images (
    credential_id TEXT NOT NULL
        REFERENCES provider_credentials_images(id) ON DELETE CASCADE,
...
INSERT INTO provider_credential_models_images (credential_id, upstream_model, created_at)
SELECT credential_id, upstream_model, created_at
FROM provider_credential_models;

CREATE TABLE model_routes_images (
    id TEXT PRIMARY KEY,
...
INSERT INTO model_routes_images (
    id, public_model, ingress_protocol, fallback_on_rate_limit, enabled,
...
CREATE TABLE route_targets_images (
    id TEXT PRIMARY KEY,
    model_route_id TEXT NOT NULL
        REFERENCES model_routes_images(id) ON DELETE CASCADE,
    provider_endpoint_id TEXT NOT NULL
        REFERENCES provider_endpoints_images(id) ON DELETE RESTRICT,
    upstream_model TEXT NOT NULL CHECK (
        upstream_model = trim(upstream_model)
...
INSERT INTO route_targets_images (
...
CREATE TABLE request_logs_images (
    ...
    provider_endpoint_id TEXT
        REFERENCES provider_endpoints_images(id) ON DELETE SET NULL,
    credential_id TEXT
        REFERENCES provider_credentials_images(id) ON DELETE SET NULL,
...
INSERT INTO request_logs_images (
    ...
    thinking_level, is_stream, created_at, client_ip
)
SELECT
    request_id, started_at_ms, config_revision, gateway_api_key_id, ingress_protocol,
    operation, public_model, provider_endpoint_id, credential_id, oauth_account_id,
...
CREATE TABLE request_attempts_images (
    request_id TEXT NOT NULL
        REFERENCES request_logs_images(request_id) ON DELETE CASCADE,
    attempt_no INTEGER NOT NULL CHECK (attempt_no >= 1),
    route_target_id TEXT
        REFERENCES route_targets_images(id) ON DELETE SET NULL,
    credential_id TEXT
        REFERENCES provider_credentials_images(id) ON DELETE SET NULL,
    oauth_account_id TEXT
        REFERENCES oauth_accounts(id) ON DELETE SET NULL,
...
INSERT INTO request_attempts_images (
...
ALTER TABLE provider_endpoints_images RENAME TO provider_endpoints;
ALTER TABLE provider_credentials_images RENAME TO provider_credentials;
ALTER TABLE provider_credential_models_images RENAME TO provider_credential_models;
ALTER TABLE model_routes_images RENAME TO model_routes;
ALTER TABLE route_targets_images RENAME TO route_targets;
ALTER TABLE request_logs_images RENAME TO request_logs;
ALTER TABLE request_attempts_images RENAME TO request_attempts;
...
CREATE TEMP TABLE migration_0029_foreign_key_guard (
    invalid INTEGER NOT NULL CHECK (invalid = 0)
);
INSERT INTO migration_0029_foreign_key_guard (invalid)
SELECT 1 FROM pragma_foreign_key_check;
DROP TABLE migration_0029_foreign_key_guard;
