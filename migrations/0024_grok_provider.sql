...
CREATE TABLE provider_endpoints_grok (
    id TEXT PRIMARY KEY,
...
INSERT INTO provider_endpoints_grok (
...
CREATE TABLE provider_credentials_grok (
    id TEXT PRIMARY KEY,
    provider_endpoint_id TEXT NOT NULL
        REFERENCES provider_endpoints_grok(id) ON DELETE RESTRICT,
...
INSERT INTO provider_credentials_grok (
...
CREATE TABLE provider_credential_models_grok (
    credential_id TEXT NOT NULL
        REFERENCES provider_credentials_grok(id) ON DELETE CASCADE,
...
INSERT INTO provider_credential_models_grok (credential_id, upstream_model, created_at)
SELECT credential_id, upstream_model, created_at
FROM provider_credential_models;

CREATE TABLE route_targets_grok (
    id TEXT PRIMARY KEY,
...
INSERT INTO route_targets_grok (
...
CREATE TABLE request_logs_grok (
    ...
    provider_endpoint_id TEXT
        REFERENCES provider_endpoints_grok(id) ON DELETE SET NULL,
    credential_id TEXT
        REFERENCES provider_credentials_grok(id) ON DELETE SET NULL,
...
CREATE TABLE request_attempts_grok (
    request_id TEXT NOT NULL
        REFERENCES request_logs_grok(request_id) ON DELETE CASCADE,
    attempt_no INTEGER NOT NULL CHECK (attempt_no >= 1),
    route_target_id TEXT
        REFERENCES route_targets_grok(id) ON DELETE SET NULL,
    credential_id TEXT
        REFERENCES provider_credentials_grok(id) ON DELETE SET NULL,
    oauth_account_id TEXT
        REFERENCES oauth_accounts(id) ON DELETE SET NULL,
...
INSERT INTO request_attempts_grok (
...
ALTER TABLE provider_endpoints_grok RENAME TO provider_endpoints;
ALTER TABLE provider_credentials_grok RENAME TO provider_credentials;
ALTER TABLE provider_credential_models_grok RENAME TO provider_credential_models;
ALTER TABLE route_targets_grok RENAME TO route_targets;
ALTER TABLE request_logs_grok RENAME TO request_logs;
ALTER TABLE request_attempts_grok RENAME TO request_attempts;
...
CREATE TEMP TABLE migration_0024_foreign_key_guard (
    invalid INTEGER NOT NULL CHECK (invalid = 0)
);
INSERT INTO migration_0024_foreign_key_guard (invalid)
SELECT 1 FROM pragma_foreign_key_check;
DROP TABLE migration_0024_foreign_key_guard;
