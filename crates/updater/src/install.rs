...
pub(crate) async fn replace_from_release(
    ...
    extract_and_replace(
        &archive_path,
        &temporary.path().join("any2api.new"),
        executable_path,
    )
}
...
fn extract_and_replace(
    ...
    let current_metadata = executable_path.metadata().map_err(|error| {
    ...
    let directory = File::open(parent)
        .map_err(|error| install_failed(format!("failed to open executable directory: {error}")))?;
    std::fs::rename(staged_path, executable_path).map_err(|error| {
        install_failed(format!("failed to replace current executable: {error}"))
    })?;
    if let Err(error) = directory.sync_all() {
        tracing::warn!(%error, "updated executable directory could not be synced after replacement");
    }
    Ok(())
}
...
pub(crate) fn extract_and_replace_for_test(
    ...
    extract_and_replace(archive_path, staged_path, executable_path)
}
