...
use crate::{
    ...
    api::{EndpointNetworkPolicy, TransportProxy, TransportRequest},
};
...
fn client_identity_is_decoupled_from_dns_but_pinned_per_origin() {
    ...
    manager
        .warm_client_for_request(
            ...
            &strict_request_to("https://one.invalid/v1"),
        )
        .expect("first strict direct client");
    manager
        .warm_client_for_request(
            TransportProxy::new(&direct, None),
            &strict_request_to("https://two.invalid/v1"),
        )
    ...
    manager
        .warm_client_for_request(
            TransportProxy::new(&direct, None),
            &request_to("https://three.invalid/v1"),
        )
