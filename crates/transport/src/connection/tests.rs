...
pub(crate) struct TestTlsIdentity {
    pub(crate) client_certificate: Vec<u8>,
...
pub(crate) async fn spawn_https_response(
...
pub(crate) async fn spawn_tls_handshake_endpoint(
    server_config: Arc<ServerConfig>,
) -> std::net::SocketAddr {
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
...
pub(crate) async fn spawn_rejecting_connect_proxy(
    status: StatusCode,
) -> (std::net::SocketAddr, oneshot::Receiver<String>) {
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
...
pub(crate) fn network_proxy(address: std::net::SocketAddr) -> ProxyProfile {
    let address =
...
pub(crate) async fn collect_body(mut response: TransportResponse) -> Bytes {
