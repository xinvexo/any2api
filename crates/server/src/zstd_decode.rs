...
const MAX_CONCURRENT_ZSTD_DECODES: usize = 2;
...
impl ZstdDecoder {
    pub(crate) fn new(lifecycle: ProcessLifecycle) -> Self {
        Self::with_limit(lifecycle, MAX_CONCURRENT_ZSTD_DECODES)
    }
    ...
    pub(crate) async fn decode(&self, bytes: Bytes) -> Result<Bytes, ZstdDecodeError> {
        self.run(move || decode(bytes))
            .await
            .map_err(|_| ZstdDecodeError::TaskFailed)?
    }

    async fn run<F, T>(&self, task: F) -> Result<T, JoinError>
    where
...
mod tests {
    ...
    use super::{
        MAX_CONCURRENT_ZSTD_DECODES, STANDARD_PUBLIC_REQUEST_BODY_LIMIT_BYTES, ZstdDecodeError,
        ZstdDecoder, decode,
    };
    ...
    async fn concurrent_blocking_work_is_limited() {
        ...
        let decoder = ZstdDecoder::new(lifecycle.clone());
        let (started_sender, mut started_receiver) = tokio::sync::mpsc::unbounded_channel();
        let mut releases = Vec::new();
        let mut tasks = Vec::new();

        for id in 0..MAX_CONCURRENT_ZSTD_DECODES {
        ...
        for _ in 0..MAX_CONCURRENT_ZSTD_DECODES {
            started_ids.push(
