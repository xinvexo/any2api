...
fn change_notifications(
    ...
    stream::unfold(
        (request_logs, system_logs, 0_u8),
        |(mut request_logs, mut system_logs, initial)| async move {
            let (name, epoch, next_initial) = match initial {
                0 => (REQUEST_LOGS_CHANGED, *request_logs.borrow_and_update(), 1),
                1 => (SYSTEM_LOGS_CHANGED, *system_logs.borrow_and_update(), 2),
                _ => {
                    tokio::select! {
                        changed = request_logs.changed() => {
                            changed.ok()?;
                            (REQUEST_LOGS_CHANGED, *request_logs.borrow_and_update(), 2)
                        }
                        changed = system_logs.changed() => {
                            changed.ok()?;
                            (SYSTEM_LOGS_CHANGED, *system_logs.borrow_and_update(), 2)
                        }
                    }
                }
            };
            Some((
                ...
                (request_logs, system_logs, next_initial),
            ))
        },
    )
}
...
mod tests {
    ...
    use super::{LogChange, REQUEST_LOGS_CHANGED, SYSTEM_LOGS_CHANGED, change_notifications};
    ...
    async fn stream_starts_with_current_epochs_and_then_emits_changes() {
        ...
        let stream = change_notifications(request_receiver, system_receiver);
