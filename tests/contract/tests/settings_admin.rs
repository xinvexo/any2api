...
async fn settings_api_exposes_defaults_overrides_and_effective_values() {
    ...
    let header_timeout = find_setting(&initial, "network.request_header_timeout");
    assert_eq!(header_timeout["value_type"], "duration_secs");
    assert_eq!(header_timeout["default_value"], 30);
    assert_eq!(header_timeout["min_value"], 1);
    assert_eq!(header_timeout["max_value"], 86_400);
    assert_eq!(header_timeout["apply_mode"], "restart_required");
    assert_eq!(header_timeout["web_group"], "入站网络");
    let body_idle_timeout = find_setting(&initial, "network.request_body_idle_timeout");
    assert_eq!(body_idle_timeout["value_type"], "duration_secs");
    assert_eq!(body_idle_timeout["default_value"], 60);
    assert_eq!(body_idle_timeout["min_value"], 1);
    assert_eq!(body_idle_timeout["max_value"], 86_400);
    assert_eq!(body_idle_timeout["apply_mode"], "restart_required");
    assert_eq!(body_idle_timeout["web_group"], "入站网络");
