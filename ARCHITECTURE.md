...
- 工具栏提供默认开启的“显示管理操作”开关；关闭时由服务端排除 `/api/admin` 及其子路径产生的日志，切换后关闭当前详情并回到最新第一页。该选择只在当前页面挂载期间有效，不写入浏览器存储，也不改变采集、保留与“清理历史日志”的范围；
