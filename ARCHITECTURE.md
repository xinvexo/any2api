...
→ 编译候选 PublishedSnapshot 并预创建所有可能失败的本地资源
→ 构造 PreparedPublish，预分配 Runtime 对象和 Registry 容量
→ 分配单调 config_revision
→ Commit
→ 执行 PreparedPublish 中无失败的 RuntimeRegistry reconcile
...
任何校验、编译、内存预留或资源创建失败都在 Commit 前终止。Commit 后只执行无 I/O、无 `Result` 的 prepared reconcile 与一次 ArcSwap，防止 SQLite 与运行时配置分裂。
...
| 设置 | 默认值 |
|---|---:|
| `affinity.soft.enabled` | `true` |
| `affinity.soft.mode` | `prefer` |
| `affinity.soft.ttl` | `60m` |
| `affinity.hard.ttl` | `24h` |
| `affinity.soft.prefer_wait_timeout` | `2s` |
| `affinity.fixed_wait_timeout` | `30s` |
...
| 设置 | 默认值 |
|---|---:|
| `scheduler.on_saturated` | `wait` |
| `scheduler.queue_timeout` | `30s` |
| `scheduler.max_waiting_requests` | `128` |
| `scheduler.fallback_on_saturation` | `false` |
| `scheduler.auxiliary_global_concurrency` | `32` |
| `scheduler.auxiliary_per_credential_concurrency` | `4` |
...
| `retry.max_credential_switches` | `2` |
| `retry.max_same_credential_retries` | `1` |
| `retry.precommit_total_budget` | `20s` |
| `retry.base_delay` | `250ms` |
| `retry.max_delay` | `2s` |
| `retry.jitter_ratio` | `20%` |
| `cooldown.rate_limit_fallback` | `60s`，优先服从 `Retry-After` |
| `cooldown.model_unsupported` | `1h` |
| `cooldown.permission_denied` | `15m` |
| `cooldown.transient_endpoint` | `15s` |
| `breaker.endpoint.failure_threshold` | `30s` 内 `3` 次 |
| `breaker.endpoint.open_duration` | `15s` |
| `breaker.proxy.failure_threshold` | `30s` 内 `3` 次 |
| `breaker.proxy.open_duration` | `30s` |
| `breaker.half_open_max_probes` | `1` |
...
- 主密钥必须稳定保存；
- 禁止在检测到已有密文时临时生成新主密钥；
- 首批使用一个数据库外的本地主密钥文件或容器 Secret，至少 256 位随机值；
- 启动时主密钥缺失或错误则直接失败，不自动生成替代密钥；
...
这些设置进入同一 SettingRegistry，可在本机管理面修改并按需热更新；改变监听地址或 TLS 绑定等必须重启的设置需明确标记 `restart_required`。启用远程监听而未启用 TLS 是受支持配置，不视为配置错误。
...
1. 主密钥首次启动时是自动生成到本地受限权限文件，还是要求用户通过环境变量/容器 Secret 提供。
2. 未来 Codex、Claude OAuth2 JSON 文件的精确 Schema、兼容来源和字段版本。
3. 首个正式版本完成后，是否继续实现 Codex Responses 与 Claude Messages 双向跨协议路由。
4. 首个正式版本完成后，是否另行实现 Codex WebSocket。
