...
40. Codex 本机额度遥测的官方 observation 与本地 RequestLog 必须使用同一个 RequestTelemetry 进程内单调位置划分区间；墙钟只用于展示和诊断，不能决定样本归属。primary usage Body 完整接收后、任何 supplement/reset-credit 查询前立即建立 observation fence；fence 之前的 OAuth RequestLog 必须已持久化或留下明确 coverage gap，fence 之后的记录不得进入当前样本。小于探测阈值的干净官方增量只推进 reset 用的 last observation，不推进 sample anchor；达到探测阈值但未达铸造阈值的干净区间把 fence 验证过的累计成本存入区段进度并继续累计，容量样本只能在累计增量达到两级铸造阈值时铸造。gap、未计价、查询失败、reset 或已消费的候选区间重新锚定前，必须先打捞已验证且达到打捞阈值的干净前缀。正常 rollover 保留 accepted samples 作为独立 `inherited` 降级先验，不得冒充 stable。容量学习是方向性的：候选是容量下界，低于上簇带宽的候选与无本地成本的官方增量只推进低样本连击并压低置信度，不修改容量；高于带宽的候选进入单槽待确认，只有两个自身一致的高样本才向上收养；没有 credential 身份、窗口形态或显式 reset 等 capacity-signature 证据时容量永不自动下调。完整决策见 `docs/adr/0141-monotonic-codex-quota-observation-and-relearning.md`、`docs/adr/0142-accumulated-segments-and-inheritable-quota-capacity.md` 与 `docs/adr/0143-directional-codex-capacity-learning.md`。
...
真实公共请求只有在选中 OAuthAccount 且 Attempt 已进入 Transport 后才记录额度活动；buffered 请求在 Attempt 结算时记录，streaming 请求在 EOF、错误、断连或 Drop 时由同一资源 Guard 只记录一次。Guard 只负责调度已有的活动驱动额度刷新。Provider 在最终请求准备阶段按精确模型与 `standard`/`fast` 速度档冻结版本化 Codex Credits 费率；RequestLog 完成时只要存在完整 input/output usage，就把输入、缓存输入和输出换算为整数 nano-Credits，与状态、取消、错误或 HTTP 结果无关。cache usage 缺失按零处理；未知模型、未知 tier 或缺 usage 保持未计价。费率在请求时冻结，禁止未来用新费率重算旧 Token；没有官方 Codex quota cache-write 倍率证据时不纳入 cache write。

容量估算以同一 quota epoch 内的 sample anchor 与当前权威 observation 为区间，而不是要求从官方窗口起点拥有完整日志。RequestTelemetry 为每个完成的 OAuth RequestLog 在有界入队前分配当前进程内单调 sequence；请求启动时若日志 policy 已关闭，完成时仍推进 sequence 与遗漏计数但不持久化。primary usage Body 完整接收后、任何 supplement/reset-credit 查询前，通过与 RequestLog 入队共用的短临界区捕获当前 sequence、墙钟和 coverage checkpoint。barrier 入队时必须冻结 policy generation 与 queue-drop/omission counter，Worker 提交 fence 前已接受日志后只补齐这些日志的 storage-failure/prune counter，禁止 barrier 后的新 loss 被旧边界吸收。fence 之后完成的日志使用更大 sequence，即使在额度查询其余阶段已经落库也不得进入当前样本。样本 SQL 只读取同一 process ID 中 `(sample_anchor.sequence, current.sequence]` 的冻结 Credits；`started_at_ms + latency_ms` 继续用于日志展示，不再承担额度区间成员判断。首次 observation 同时建立 last observation 与 sample anchor；后续 last observation 每次推进，用于 reset/jitter 判断，而 `delta_used_percent = current - sample_anchor` 小于 `0.5` 个百分点的探测阈值且 coverage 完整时保留 sample anchor，使零增量、百分比量化和连续小增量可以累积。达到探测阈值后查询整个 sequence 区间：coverage 完整、没有未计价记录且成本为正的干净区间若未达铸造阈值，把查询结果写入区段进度（最近一次干净探测的官方百分比、墙钟、fence 验证过的累计成本、计价请求数与费率卡），保留 sample anchor 并报告 `accumulating`；达到铸造阈值才生成 `capacity_sample = local_credits × 100 / delta_used_percent`，然后重新锚定并清空进度。铸造阈值两级：accepted samples 为空时 `1.5` 个百分点，否则 `5.0` 个百分点，使样本分母显著大于官方量化与上游入账时序的绝对误差。gap、查询失败、未计价或已经分类的候选区间同样重新锚定，禁止残缺区间继续累计；但重新锚定前必须先打捞区段进度：已存干净前缀的累计增量达到 `1.5` 个百分点时，按该前缀铸造一个经过相同鲁棒分类的容量样本，因为其成本在自身探测时已通过 fence 验证，后续缺口不追溯污染。查询后的 checkpoint 只检测 SQL 期间可能删除已持久化区间行的 prune；状态必须保存 observation fence 自身的 checkpoint，fence 后请求的队列/写入结果只由下一 interval 消费。barrier 不可用或超时返回不完整 observation。公开请求路径仍只做短临界区与有界 `try_send`，绝不等待 DB。跨进程、日志启停、丢弃、写失败或清理导致的相关区间直接标记不完整，不用低可信数值污染 estimator。

每个账号窗口显式维护 quota epoch。官方 `reset_at` 在两个连续 last observation 之间发生超过 60 秒的实质变化、`reset_at` 的有无状态变化、没有 reset identity 时跨过完整窗口、相邻使用率下降超过 `0.5` 个百分点、OAuth 稳定身份/`account_generation` 变化、无法证明同一 generation 或显式 reset credit 成功都会结束旧 epoch；reset 判断禁止拿可能跨多个小增量的 sample anchor 代替 last observation。同一窗口不超过 60 秒的相邻 `reset_at` 秒级漂移只更新 last observation，`50.0 → 49.8` 同样只作为 jitter，`70 → 3` 即使没抓到 0 也必须结束旧 epoch，使既有样本降级为 `inherited` 先验。窗口最多保留最近 9 个有效容量样本并使用 median，每个样本携带铸造时的 epoch；至少 3 个一致样本、相对 MAD 不超过 20% 且存在当前 epoch 样本才是 `stable`，存在当前 epoch 样本但不满足前述条件是 `learning`，样本全部来自更早 epoch 时是独立的 `inherited` 降级先验，已知 telemetry gap/外部消费/异常值后或低样本连击达到 2 时是 `degraded`。容量学习方向性执行：候选是容量下界，accepted samples 构成自身一致的上簇，接受带宽为簇中心 ±25%。带宽内候选进入簇并清零低样本连击；低于带宽的候选与有官方增量但无本地成本的区间标记外部消费、推进低样本连击，不进入簇也不修改容量；高于带宽的候选进入单槽待确认，仅当下一个高候选与其比值不超过 1.25 时两者整体向上收养为新簇，否则替换待确认槽。telemetry gap、查询失败、未计价区间和 epoch 边界清空待确认槽；低候选不清空。没有 credential 身份指纹变化、窗口 ID/类型/时长变化或显式 reset 等 capacity-signature 证据时，容量永不自动下调；未重新登录的套餐降级由持续 `degraded` 提示而不是被学入。当前已用/剩余只用 accepted sample median 与官方当前百分比投影。Estimator state、last observation、sample anchor、区段进度、checkpoint、当前 epoch、带 epoch 标记的 accepted samples、待确认高样本与低样本连击随 v7 quota snapshot 持久化；进程重启保留可证明同 epoch 的容量样本，但跨重启区间不产生新样本（重启前已验证的区段进度仍可打捞）。正常 rollover 保留 accepted samples 作为 `inherited` 先验：上游没有可验证的 Codex plan/capacity signature，因此该先验不得伪装为当前 Stable，必须由当前 epoch 新样本确认，向上修正同样只依赖两个一致高样本。credential 身份指纹或窗口 ID/类型/时长变化仍清空全部样本；显式 reset 删除旧 snapshot，下一次刷新冷启动。所有估算与置信诊断只用于管理展示，不参与健康、路由、RPM、计费或任何运行态恢复。完整边界见 `docs/adr/0138-epoch-interval-codex-quota-telemetry.md`、`docs/adr/0140-codex-quota-reset-at-jitter-tolerance.md`、`docs/adr/0141-monotonic-codex-quota-observation-and-relearning.md`、`docs/adr/0142-accumulated-segments-and-inheritable-quota-capacity.md` 与 `docs/adr/0143-directional-codex-capacity-learning.md`。
...
- Codex 账号可显式刷新上游额度窗口、购买 Credits 和 reset credit 次数；购买 Credits 的标签固定为 `Credits`，有限余额按版本化官方费率卡换算成最多四位小数的美元等值，悬浮显示原始 Credits 和 `25 Credits = $1`，不与 reset credit 合并。5 小时/7 天窗口在对应百分比同一行展示 snapshot-interval estimator：cold start/无样本时显示“学习中”，有样本时只用紧凑 `$已用/$总量`，并在悬浮文本展开 `learning/stable/degraded`、epoch、样本数、离散度、最近区间状态、Credits 容量、时间、未计价记录与费率卡。美元值只是在 Web 由 Credits 派生的标准费率卡等值，不能标成上游余额。额度最后更新时间保留月、日和时分秒，不重复显示当前语境中多余的年份。只有同次查询确认 reset credit 剩余次数大于 0 时才显示可用的“重置额度”操作，提交前必须二次确认，成功后立即重新查询；
...
Codex Credits = Upstream hasCredits / unlimited / balance + Versioned 25 Credits Per USD Display + Never Conflated With Reset Credits
Quota Estimate = Same-Epoch Snapshot Delta + Complete Interval Frozen Credits + Robust Median + Confidence + Display Only
