...
每次刷新都从 SQLite 重新计算整个官方周期的本地总和并覆盖保存，不增加相邻刷新小区间，不跨官方重置累计，也不维护 EMA、样本、异常值或累计区间数。`estimated_used_credits` 直接等于当前周期 `local_cost_nanos` 的 Credits 换算；当有限非负 `used_percent >= 2.0`、本地成本为正且周期身份可比时，容量按 `local Credits × 100 / used_percent` 计算，剩余量按容量减本地已用计算；不满足条件时仍返回本地已用，但容量和剩余量为未知，防止整数百分比量化、上游异步记账或跨身份数据被放大。官方周期缺少正数时长或非负 reset 时间时不生成本机统计。凭据身份或已知订阅层级在已有周期中改变时，继续查询并返回本地周期总和，但直到新 `reset_at` 周期才恢复容量推算，禁止用混合身份周期反推总量。Estimator v10 状态只持久化凭据指纹、可选订阅层级，以及每个窗口的键、周期起止身份、整数 nano-Credits 总和和容量可比标记；Migration 0036 清除无法还原单一官方周期的 v9 estimator state，同时保留最后一次官方 usage。生产 Rust 只接受 v10 当前结构。已有 snapshot 读取、校验或周期 SQL 查询失败时 refresh 必须失败并保留原记录。本机统计只用于管理展示，不参与健康、路由、RPM、计费或运行态恢复。
