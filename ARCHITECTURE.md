...
40. Codex 本机额度遥测的官方 observation 与本地 RequestLog 必须使用同一个 RequestTelemetry 进程内单调位置划分区间；墙钟只用于展示和诊断，不能决定样本归属。primary usage Body 完整接收后、任何 supplement/reset-credit 查询前立即建立 observation fence；fence 之前的 OAuth RequestLog 必须已持久化或留下明确 coverage gap，fence 之后的记录不得进入当前样本。小于探测阈值的干净官方增量只推进 reset 用的 last observation，不推进 sample anchor；达到探测阈值但未达铸造阈值的干净区间把 fence 验证过的累计成本存入区段进度并继续累计，容量样本只能在累计增量达到两级铸造阈值时铸造。gap、未计价、查询失败、reset 或已消费的候选区间重新锚定前，必须先打捞已验证且达到打捞阈值的干净前缀。正常 rollover 保留 accepted samples 作为独立 `inherited` 降级先验，不得冒充 stable；纯继承模型允许由 2 个自身一致的同侧候选整体替换，已有当前 epoch 样本确认的模型仍需连续 4 个,单个异常值不得触发替换。完整决策见 `docs/adr/0141-monotonic-codex-quota-observation-and-relearning.md` 与 `docs/adr/0142-accumulated-segments-and-inheritable-quota-capacity.md`。
...
每个账号窗口显式维护 quota epoch。官方 `reset_at` 在两个连续 last observation 之间发生超过 60 秒的实质变化、`reset_at` 的有无状态变化、没有 reset identity 时跨过完整窗口、相邻使用率下降超过 `0.5` 个百分点、OAuth 稳定身份/`account_generation` 变化、无法证明同一 generation 或显式 reset credit 成功都会结束旧 epoch；reset 判断禁止拿可能跨多个小增量的 sample anchor 代替 last observation。同一窗口不超过 60 秒的相邻 `reset_at` 秒级漂移只更新 last observation，`50.0 → 49.8` 同样只作为 jitter，`70 → 3` 即使没抓到 0 也必须结束旧 epoch，使既有样本降级为 `inherited` 先验。窗口最多保留最近 9 个有效容量样本并使用 median，每个样本携带铸造时的 epoch；至少 3 个一致样本、相对 MAD 不超过 20% 且存在当前 epoch 样本才是 `stable`，存在当前 epoch 样本但不满足前述条件是 `learning`，样本全部来自更早 epoch 时是独立的 `inherited` 降级先验，已知 telemetry gap/外部消费/异常值后是 `degraded`。稳定分布形成后，低于 median 50% 的候选仍优先标记外部消费，其他远离 `max(3×MAD, 25%×median)` 或两倍中心范围的候选标记异常；两类拒绝候选都进入最多 4 项的同侧竞争簇。有当前 epoch 样本确认的模型只有连续 4 个同侧候选自身相对 MAD 不超过 20% 时才被竞争簇整体替换；纯继承模型由 2 个自身一致的同侧候选即可替换。正常接受的候选、方向反转、gap、reset 或非法区间清空竞争簇，单个或离散异常值不能触发重学习。当前已用/剩余只用 accepted sample median 与官方当前百分比投影。Estimator state、last observation、sample anchor、区段进度、checkpoint、当前 epoch、带 epoch 标记的 accepted samples 和竞争簇随 v7 quota snapshot 持久化；进程重启保留可证明同 epoch 的容量样本，但跨重启区间不产生新样本（重启前已验证的区段进度仍可打捞）。正常 rollover 保留 accepted samples 作为 `inherited` 先验：上游没有可验证的 Codex plan/capacity signature，因此该先验不得伪装为当前 Stable，必须由当前 epoch 新样本确认或由一致反向候选失效。credential 身份指纹或窗口 ID/类型/时长变化仍清空全部样本；显式 reset 删除旧 snapshot，下一次刷新冷启动。所有估算与置信诊断只用于管理展示，不参与健康、路由、RPM、计费或任何运行态恢复。完整边界见 `docs/adr/0138-epoch-interval-codex-quota-telemetry.md`、`docs/adr/0140-codex-quota-reset-at-jitter-tolerance.md`、`docs/adr/0141-monotonic-codex-quota-observation-and-relearning.md` 与 `docs/adr/0142-accumulated-segments-and-inheritable-quota-capacity.md`。
