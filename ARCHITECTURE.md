...
19. 不提供通用配置或 Secret 导入导出；OAuth2 登录是独立工具，只负责一次性生成 Provider 认证 JSON 文件，不创建或修改 `ProviderCredential`。
...
- OAuth2 登录产生的认证 JSON 只允许停留在一次性 HTTP 响应和浏览器下载内存中；服务端不持久化、不记录日志，也不提供二次读取或导出；
...
flowchart LR
    ...
    WEB --> OAUTH["Standalone OAuth Login"]
    OAUTH --> OAUTH_PROVIDER["Provider Authorization / Token Endpoint"]
    OAUTH_PROVIDER --> OAUTH_FILE["One-Time JSON Download"]
...
│  │     ├─ oauth/              # 独立 PKCE session、Token exchange、认证文件生成
...
- 配置编译器只接受 `credential_kind=api_key`；OAuth2 登录结果不进入 Credential 配置、Vault、模型发现或调度路径；
...
- 模型集合变化增加 `config_version`，不增加 `secret_version` 或 `credential_generation`；API Key 轮换会清空旧模型集合；
...
- Web 与管理 API 通过独立 OAuth start/exchange 端点生成认证文件；Provider 列表和 Credential DTO 不展示 OAuth 类型或 OAuth 入口；
- OAuth session、state 和 PKCE verifier 只保存在内存；Token 只存在于兑换栈和一次性下载响应中，响应完成后服务端不保留；
- 未来若增加 Provider 专用 OAuth2 JSON 导入，必须单独设计导入目标和 Schema，不能复用本登录工具的会话或建立隐式 Credential。
...
CLIProxyAPI 和 sub2api 会把它暴露为 `/v1/responses` 的兼容别名，主要用于连接 ChatGPT/Codex OAuth 数据面。any2api 当前只提供独立 OAuth2 认证文件生成，不把该文件接入 `ProviderCredential` 或公开请求数据面；是否增加此兼容入口以及如何导入对应认证文件属于独立后续切片。
...
trait ProviderDriver: Send + Sync {
    ...
    fn oauth_token_request(&self, grant: OAuthGrant, code: &str, state: Option<&str>, code_verifier: Option<&str>) -> Result<OAuthRequestPlan>;
    fn parse_oauth_token(&self, body: &[u8]) -> Result<OAuthTokenMaterial>;
    fn classify_error(&self, operation: ProtocolOperation, meta: &UpstreamResponseMeta, bounded_body: &[u8]) -> UpstreamErrorClassification;
}
...
具体方法可以在实现阶段调整，但职责边界不可合并为一个万能 Driver。Provider 只处理供应商、Endpoint、认证和错误差异，ProtocolAdapter 负责线协议双向编解码，Runtime 负责网络与编排。OAuth 方法只供独立认证文件工具调用，不代表 Provider 数据面支持 OAuth Credential。`ProviderRegistry` 和 `ProtocolRegistry` 由 `app` 在编译期静态注册，Runtime 只依赖接口和 CapabilitySet。
...
Codex 与 Claude 的上游 `ProviderCredential` 当前都只支持 API Key。独立 OAuth2 工具生成的 JSON 文件不会进入 Runtime generation、Provider Header 注入或配置发布链；普通 API Key 管理端点也不接受 OAuth JSON。
...
- 同一个 Credential ID 在配置代际之间复用稳定的容量计数；
...
所有尝试共享同一个绝对 deadline。排队、Session Lock、退避和请求上游都消耗该 deadline；开始下一次尝试前必须释放上一 Credential 的 Permit。
...
- 实时容量、健康和冷却从稳定 RuntimeRegistry 读取；
...
### 16.3 独立 OAuth2 认证文件生成

当前实现 Codex 与 Claude 的交互式 OAuth2 登录，唯一产物是一次性下载的 Provider 认证 JSON 文件。该流程与 Provider Endpoint、Credential、代理、最大并发、模型选择、配置 revision 和 Vault 完全无关。
...
→ Runtime 使用本机 DIRECT 执行 Token exchange
→ Provider Driver 解析 OAuthTokenMaterial
→ Runtime 序列化 Provider 专用认证文件
→ HTTP 响应以 attachment + Cache-Control: no-store 一次性交付
→ 浏览器立即下载，服务端不保留 Token 或文件
```

OAuth session 最多同时 64 个，10 分钟过期，只在内存存在。Codex 与 Claude 的 authorize/token Endpoint、Client ID 和 localhost Redirect URI 由各自 Driver 固定。交换请求不读取全局代理或 Provider 配置，也不提供 Endpoint、代理、并发、启用状态或模型字段。

认证文件 Schema 以 CLIProxyAPI 的 Provider token storage 为基线，并与 new-api 的 Codex OAuth JSON parser、Sub2API 的 Codex auth.json 导入器保持字段兼容：
...
时间字段使用 UTC RFC 3339。Provider 没有返回的可选字段编码为空字符串，不伪造值。成功兑换会在开始网络请求前消费 session；同一 session 不能再次下载。原始 callback URL、authorization code、access token、refresh token、ID token 和生成的 JSON 不进入日志、SQLite、Vault、PublishedSnapshot、React Query、浏览器存储或页面长期 DOM。服务端不提供认证文件列表、二次读取、刷新 Worker 或 Secret 导出端点。
...
Credential Secret 和代理密码使用版本化 AEAD 加密保存。OAuth2 登录产物不进入 Vault。
...
- 使用明文 HTTP 时，Web 必须持续显示安全警告，明确管理员密码、会话 Cookie 和 OAuth2 JSON 下载可能被同网络中的攻击者截获；
- 只有实际使用反向代理且其 CIDR 在可信列表中时，才接受 Forwarded/X-Forwarded-* 管理来源信息；
- OAuth2 JSON 可以从已认证的 HTTP 或 HTTPS 管理面下载；使用 HTTP 时不额外阻止，但必须显示明文传输警告；
...
- 明文远程 HTTP 会话响应必须标记风险状态，React 管理壳在整个已登录会话持续显示密码、Cookie 和 OAuth2 JSON 下载可能被截获的警告；该警告不阻止操作；
...
- 只选择 Codex 或 Claude，不选择 Endpoint、代理、最大并发、启用状态或模型；
- 打开授权页面后，允许粘贴完整 localhost callback URL；
- 授权成功后立即下载 `codex-auth.json` 或 `claude-auth.json`；
- 页面不长期展示 Token，不保存下载内容，也不跳转到 Provider 或模型选择流程；
- session ID、state、authorization code、callback URL 和下载 Blob 不进入地址栏、React Query、Mutation Cache、localStorage 或 sessionStorage。
...
数据目录只允许一个 any2api 进程持有实例锁。获取失败时直接退出，避免两个进程分别维护 `in_flight`、OAuth 登录 session 和内存会话状态。
...
流式 Body、OAuth token exchange、健康检查和日志写入任务统一交给进程级 TaskTracker 管理。停机完成后，所有并发、队列、冷却和会话状态直接丢弃。
...
- 独立一级菜单与页面；
- 内存 PKCE/session、DIRECT token exchange 和一次性 JSON 文件下载；
- Token 不进入 Vault、ProviderCredential 或后台刷新；
...
1. 未来是否允许把独立 OAuth2 登录生成的认证文件重新导入 any2api 数据面，以及对应的明确 Credential 语义。
...
OAuth Login ──X ProviderEndpoint / ProviderCredential / Proxy / Models
OAuth Session/PKCE ──> Memory Only + 10 Minute TTL + One-Time Exchange
OAuth Token ──> One-Time Provider JSON Download + No Persistence
...
OAuth2 JSON Download = Provider-Specific One-Time Exception
Future OAuth2 JSON Import = Separate Undecided Feature
