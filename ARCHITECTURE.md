...
40. Codex 本机额度统计只使用官方窗口使用率和本地 SQLite 中仍保留的 RequestLog。每次成功观测都以官方 `reset_at - limit_window_seconds` 为周期起点、当前 observation fence 为终点，直接重算该 OAuthAccount 在整个官方周期内的冻结 Credits 总和；不得再把相邻刷新拆成小区间累计，也不得把不同官方重置周期混合。界面“已用”始终直接显示该周期本地总和；只有官方周期使用率达到 `2%` 后，才按 `local Credits × 100 / used_percent` 推算总量，低于门槛时只隐藏总量而不隐藏本地已用。当前进程内单调位置只负责排除 fence 之后完成的记录，墙钟完成时间负责官方周期下界和跨进程记录归属；观测必须在同一日志 Writer 排入 flush barrier 并等待完成。容量可比身份优先使用 Provider Driver 投影的稳定上游主体；能够取得稳定主体时，账号启停、OAuth Token 换代、代理选择和 `account_generation` 都不得改变额度身份。只有稳定上游主体、已知订阅层级，或缺少稳定主体时使用的保守认证身份在周期中改变，才继续返回本地已用但暂停该周期容量推算，等下一官方周期重新开始。每条 RequestLog 在 Attempt 准备时从当前 PublishedSnapshot 的 `oauth.codex.rate_card` 冻结模型 Token→Credits 费率与卡片 ID，后续改价不得重算历史日志。
...
每次刷新都从 SQLite 重新计算整个官方周期的本地总和并覆盖保存，不增加相邻刷新小区间，不跨官方重置累计，也不维护 EMA、样本、异常值或累计区间数。`estimated_used_credits` 直接等于当前周期 `local_cost_nanos` 的 Credits 换算；当有限非负 `used_percent >= 2.0`、本地成本为正且周期身份可比时，容量按 `local Credits × 100 / used_percent` 计算，剩余量按容量减本地已用计算；不满足条件时仍返回本地已用，但容量和剩余量为未知，防止整数百分比量化、上游异步记账或跨身份数据被放大。官方周期缺少正数时长或非负 reset 时间时不生成本机统计。额度身份指纹先写入 Provider，再在 Driver 能投影稳定主体时只写入该主体摘要；此路径不写入 `account_generation` 或 `token_version`，因此同一上游主体的停用后启用、代理切换、普通 Token 刷新和重新授权不破坏周期可比性。只有无法取得稳定主体时，指纹才回落到 `account_generation + token_version` 并在任一代际变化后保守暂停当前周期推算。稳定主体、回落身份或已知订阅层级在已有周期中改变时，继续查询并返回本地周期总和，但直到新 `reset_at` 周期才恢复容量推算，禁止用混合身份周期反推总量。Estimator v10 状态只持久化凭据指纹、可选订阅层级，以及每个窗口的键、周期起止身份、整数 nano-Credits 总和和容量可比标记；Migration 0036 清除无法还原单一官方周期的 v9 estimator state，同时保留最后一次官方 usage；Migration 0039 在采用稳定主体指纹时清除旧算法生成、无法安全转换的 estimator state，同样保留官方 usage，使下一次权威刷新从整周期 RequestLog 无损重算。生产 Rust 只接受 v10 当前结构。已有 snapshot 读取、校验或周期 SQL 查询失败时 refresh 必须失败并保留原记录。本机统计只用于管理展示，不参与健康、路由、RPM、计费或运行态恢复。
...
- Codex 账号可显式刷新上游额度窗口、购买 Credits 和 reset credit 次数；购买 Credits 的标签固定为 `Credits`，有限余额按 `oauth.codex.rate_card` 当前生效汇率换算成最多四位小数的美元等值，悬浮显示原始 Credits 和换算率，不与 reset credit 合并。5 小时/7 天窗口在对应百分比同一行展示本机周期统计：周期 SQL 可用时始终显示紧凑的本地已用金额；官方使用率达到 `2%` 后显示 `$已用/$总量`，不足门槛时显示 `$已用/暂无`。悬浮详情以单组信息展示已用、剩余、总量、原始 Credits 和换算率，不重复汇总值或显示 `USD 等值` 单位，不显示计算方式、小区间数、置信度、样本质量或异常诊断；官方 reset 后只展示新周期记录。美元值只是在 Web 由 Credits 派生的当前费率卡等值，不能标成上游余额。额度最后更新时间保留月、日和时分秒，不重复显示当前语境中多余的年份。只有同次查询确认 reset credit 剩余次数大于 0 时才显示可用的“重置额度”操作，提交前必须二次确认，成功后保留现有快照并立即重新查询；只有该查询拿到的官方 reset 边界才能开始下一个窗口周期；
