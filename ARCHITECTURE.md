...
真实公共请求只有在选中 OAuthAccount 且 Attempt 已进入 Transport 后才记录额度活动；buffered 请求在 Attempt 结算时记录，streaming 请求在 EOF、错误、断连或 Drop 时由同一资源 Guard 只记录一次。该 Guard 同时接收 ProtocolAdapter 已有的累计 TokenUsage；Provider 可按精确模型提供版本化官方标准输入、缓存输入和输出费率，把完整样本折算为美元成本。只有当前进程中相邻两次成功权威快照的窗口 ID、类型、时长与 reset identity 相同、使用率严格增加，且期间所有已观察 Attempt 的模型和 usage 均可完整计价时，才能用 `sample_cost × 100 / percent_delta` 生成窗口容量、已用和剩余美元等值估算。窗口 reset、百分比下降/不变、未知模型、usage 缺失与非法成本只建立新基线；估算不参与健康、路由、RPM、计费或启动恢复。活动由单个进程生命周期 Worker 按账号短 debounce 合并，并施加最小自动刷新间隔和最多 6 个并发；刷新中出现的新活动最多保留一个后续刷新。失败保留旧快照且不固定周期重试，只有后续真实活动再次调度。没有待处理活动时 Worker 不扫描账号或 SQLite，进程启动也不为闲置账号建立 quota 定时任务。成功 upsert 或 reset 删除快照后推进 quota change epoch；Token 刷新诊断写入、更新或失效后推进独立 refresh diagnostic epoch。受认证 `/api/admin/oauth/quota-events` SSE 只发送 `oauth_quota_changed` 或 `oauth_refresh_diagnostic_changed` 无业务 payload 失效事件，Web 随后分别重读 SQLite quota GET 或安全账号元数据，不由事件访问 Provider，也不在事件中携带诊断或 Secret。
...
- Codex 账号可显式刷新上游额度窗口、购买 Credits 和 reset credit 次数；购买 Credits 按上游真实 Credits 单位显示，不与 reset credit 合并。有完整相邻样本时，5 小时/7 天窗口额外显示按实际 Token 与版本化官方标准 API 价反推的容量、已用和剩余美元等值，并明确标为本机观测估算而非上游余额。只有同次查询确认 reset credit 剩余次数大于 0 时才显示可用的“重置额度”操作，提交前必须二次确认，成功后立即重新查询；
