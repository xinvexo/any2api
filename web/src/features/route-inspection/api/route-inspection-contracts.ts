...
function parseOperation(value: unknown): RouteInspectionOperation {
  ...
  return {
    operation: oneOf(operation.operation, OPERATIONS),
...
function parseCandidateGroup(value: unknown): RouteInspectionCandidateGroup {
  ...
  return {
    providerKind: oneOf(group.provider_kind, PROVIDERS),
    ...
    upstreamProtocolDialect: oneOf(group.upstream_protocol_dialect, DIALECTS),
