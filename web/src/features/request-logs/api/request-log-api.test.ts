...
test("serializes exact request log filters without client-controlled audit headers", async () => {
  ...
  await getRequestLogs("r4.cursor", {
    outcome: "failed",
  ...
  expect(fetchMock.mock.calls[0]?.[0]).toBe(
    "/api/admin/request-logs?cursor=r4.cursor&outcome=failed&public_model=claude-test&gateway_api_key_id=11111111-1111-4111-8111-111111111111",
  );
