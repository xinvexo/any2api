import type { OAuthQuotaCredits } from "../api/oauth-quota-contracts";

const CODEX_CREDITS_PER_USD_2026_08_11 = 25n;
const DISPLAY_FRACTION_DIGITS = 4;
const DISPLAY_SCALE = 10n ** BigInt(DISPLAY_FRACTION_DIGITS);

export function formatCodexCreditsUsd(balance: string) {
  const [whole, fraction = ""] = balance.split(".");
  const sourceScale = 10n ** BigInt(fraction.length);
  const credits = BigInt(`${whole}${fraction}`);
  const denominator = CODEX_CREDITS_PER_USD_2026_08_11 * sourceScale;
...
export function presentCodexCredits(credits: OAuthQuotaCredits) {
  if (credits.unlimited) return { value: "无限" };
  if (credits.hasCredits && credits.balance !== null) {
    return {
      value: formatCodexCreditsUsd(credits.balance),
      detail: `${credits.balance} Credits · 25 Credits = $1`,
    };
  }
