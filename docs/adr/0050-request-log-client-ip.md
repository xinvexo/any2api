...
- 管理请求日志列表与详情 DTO 返回必填 `client_ip`，Web 在桌面列表默认列、窄屏卡片摘要和独立详情页展示；内联展开区不重复列表已有的 IP。该字段只存在于已认证的管理响应和浏览器当前查询缓存，不进入普通结构化文件日志。
...
- Web 契约与组件测试覆盖必填地址解析、桌面列表默认列、窄屏摘要、独立详情展示，以及内联展开区不重复显示。
