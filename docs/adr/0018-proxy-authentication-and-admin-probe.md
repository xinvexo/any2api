...
- `ProxyProfile` 只保存可见的认证元数据与单调 `authentication_version`；密码按 ADR-0074 明文保存在独立 `proxy_passwords` 记录中。
- 认证状态实际发生设置、替换或清除时增加 `authentication_version` 与 `config_version`；对已经关闭认证状态的重复清除是 no-op；已开始请求的 PublishedSnapshot 继续持有其捕获的认证材料，新请求使用当前 Client 代际。
...
- Domain/Storage 测试覆盖认证版本、用户名/密码校验、认证仍存在时的重启加载、设置/替换/清除、重复清除 no-op 和 Debug 脱敏。
