...
- Provider drivers own the OAuth live-catalog scope, request and parser, but never embed a model catalog. The current upstream catalog is read at interactive login and as part of an administrator's manual quota refresh under ADR-0158, not by the scheduler or automatic quota activity.

These fixed profiles are internal routing projections only. They are never returned by, inserted into, or editable through Provider Endpoint APIs. A live catalog is a separately persisted safe management snapshot, not a Profile field: it is keyed by `Provider + directory_scope`, stores only sorted model names and a fetch time, never silently changes selected models, and a failed read preserves the previous snapshot. A batch manual quota refresh reads each shared scope at most once. Interactive login initializes selected models from a successful live catalog; imported accounts default to no local RPM limit, an empty model selection, enabled state, and `Global` proxy selection until an administrator refreshes their directory. The OAuth page may edit label, enabled state, optional RPM, proxy selection, and selected models; it does not expose Endpoint or API-key Credential forms. The routing projection accepts explicitly selected valid model names, so an administrator can save a model discovered by the live read or entered manually.
