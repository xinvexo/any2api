...
首版需要把客户端可见模型精确映射到一个或多个同协议 Provider Endpoint。Route 与 Target 如果通过多次独立管理请求保存，会产生只有 Route、没有 Target，或只保存部分 Target 的中间配置；Target ID 同时还会成为后续会话绑定的稳定身份。
...
- Target ID 由服务端生成并稳定保留。更新已有 Target 时只允许改变 `fallback_tier` 和 `enabled`；改变 Endpoint 或 `upstream_model` 必须创建新 Target ID，防止旧会话绑定被静默解释为另一上游身份。
