...
- 不在 Scheduler 或 Credential 上使用按请求数量计数的固定 `tokio::Semaphore`：调度准入只由滚动 RPM 窗口决定，否则会形成额外账号并发限制。单请求 Body/响应硬上限属于协议边界，不参与本 ADR 的候选调度。
