...
7. 带候选身份的传输、协议、预提交或 Attempt 总预算超时，以及认证、权限、额度、限流、模型/操作不可用和
   瞬态上游失败，会比较并删除仍指向该候选的提示。普通 InvalidRequest、any2api 本地后处理失败与客户端取消不删除提示，
   因为它们不能证明路径失效。OAuth 认证失败若在原账号上成功刷新 Token，同一路径提示继续保留；刷新失败
   或刷新后 Token 再次被拒绝才删除。比较删除避免旧失败并发删除后来已由另一成功请求更新的映射。配置代际
   变化后，无法匹配当前完整候选身份的记录在读取时失效。
...
- 冷却回归测试覆盖 403 淘汰旧路径、后备路径成功覆盖，以及旧路径冷却结束后同一键仍优先后备路径。
