...
- “删除失效账号”先复用逐账号额度 GET 检测当前 Provider 的完整账号集合，最多并发 6 个。只有稳定错误码 `oauth_account_authentication_failed` 能够进入删除候选；ADR-0070 将该错误的可验证来源扩展为刷新后再次 401、已被 401 拒绝且没有 refresh token，以及刷新端点明确返回永久失效码。无法识别的刷新拒绝、超时、代理/网络故障、5xx、403、额度耗尽、机器人标记和其他读取失败仍记为无法确认并保留账号。
