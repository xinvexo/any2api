...
11. Grok quota、Token 余额与账号诊断都是只读临时快照，不写 SQLite、RequestLog、OAuth JSON、PublishedSnapshot 或浏览器持久化，也不参与 RPM 或持久化账号启停。ADR-0070 与 ADR-0095 允许明确的额度耗尽或权威 Token 零余额临时影响当前账号 `routing_generation` 的路由健康，并跨仅替换认证材料的 Token refresh 保留；完整 Header 解析契约见 ADR-0060。
