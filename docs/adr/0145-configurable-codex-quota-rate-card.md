...
1. SettingRegistry 新增 `oauth.codex.rate_card`，类型为受严格 Schema 校验的 `codex_rate_card`。编译默认值
   继续表达当前已知费率，SQLite 只保存用户覆盖值，Web 在“运行保护 → Codex 额度估算”中编辑 JSON。
...
- Server/Web 测试覆盖额度响应中的展示汇率、真实 Credits 与估算共用换算、设置草稿校验和证据状态说明。
