...
5. 第 1、2 条按 `affinity.enabled` 快照值条件化，不新增设置：开关关闭（均衡模式）时按上述行为
   保证缓存连续；开启（粘性模式）时会话已钉死在单一凭据上，缓存不依赖跨凭据一致性，因此恢复
   指纹隔离——会话/追踪头（`SessionScoped` 类）在换凭据的 Attempt 上删除，数据面连接池按凭据
   代际隔离。请求压缩在两种模式下都保持关闭。
