...
`ProviderCredential` remains API-key-only and Provider management APIs/Web pages do not gain an OAuth kind. At publication time, configured Provider credentials and OAuth accounts are compiled into a common internal `RoutingCredential` projection; `enabled` controls routing eligibility without erasing the stable identity. The projection has a stable routing ID, Endpoint, actual proxy, model set, maximum concurrency, generation, and authentication material. The scheduler only consumes this projection, so both sources use the existing atomic select-and-acquire operation, load ratio, QueueTicket, unified fixed session affinity, health state, retry exclusions, response commit boundary, and Permit lifetime.

OAuth routing IDs occupy a disjoint UUID namespace from configured Credential IDs. Disabling an OAuth account only removes it from route candidates; existing bindings remain fixed and fail locally while it is unavailable. Deleting the account retires that routing credential and clears its affinity exactly like removing a configured Credential. Re-enabling the same account preserves its stable routing identity and allows its still-live in-memory bindings to resume.
