...
- `cargo xtask package` 是从当前源码生成完整单二进制的唯一正式入口：默认依次执行前端生产构建、同步内嵌目录、只读一致性复核和锁定依赖的 Rust release 构建。`--check-assets` 把同步阶段替换为只读比较，供干净 checkout 的 Release 门禁使用；可选 `--target <triple>` 只改变 Rust 目标，不改变 Web 生成链。GitHub Release 使用同一入口，不能复制一套手写步骤。
- 普通 `pnpm build` 只生成 `web/dist`，不得修改已提交资源；`pnpm build:embedded` 是供打包入口和前端维护者使用的低层构建加同步原语，不再承担完整应用打包。Web CI 与 `package --check-assets` 都先执行普通构建，再只读比较 checkout 中的 `web-assets`，避免“先覆盖再检查”的自证式门禁；本地默认打包则有意从当前 Web 源码重新生成资源后再构建 Rust 二进制。
...
- 前端变更需要同步生成产物；`cargo xtask package` 自动完成该步骤，CI 的只读一致性检查仍防止提交遗漏。内容哈希变化直接替换文件，不保留资源名兼容别名。
- 外部目录是开发和诊断入口，不是另一套正式发布模型；两种来源共享 API 隔离和 SPA 入口语义，但文件系统响应可继续使用 `tower-http` 的元数据实现。

## 后果

- 正式二进制可以离开仓库独立运行，管理页面不受当前工作目录影响。
- 干净 Rust CI、`cargo test` 和普通 `cargo build --release` 使用已提交资源，无需安装 Node；它们是 Rust 构建入口，不宣称重新生成当前 Web 源码。需要得到包含当前前后端源码的可发布二进制时使用 `cargo xtask package`，该入口要求 Node 与 pnpm。
...
- 前端同步脚本在内容或文件清单不一致时失败，并提示明确的同步命令；Web CI 与 `package --check-assets` 在不修改 checkout 产物的普通构建后执行该检查，默认打包入口在同步后再次只读复核。
