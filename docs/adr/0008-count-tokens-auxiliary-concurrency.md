...
当前统一 QueueTicket 和 SettingRegistry 尚未实现，因此本切片还不能引入独立等待队列或从 SQLite 读取覆盖值。

## 决策

- RuntimeRegistry 持有跨配置 revision 复用的 `AuxiliaryScheduler`，PublishedSnapshot 只持有该稳定句柄。
- 辅助并发默认值集中定义为全局 `32`、单 Credential `4`；使用强类型 `AuxiliaryConcurrencyLimits`，允许构造注入和运行时更新，为后续 SettingRegistry 接管保留接口。
...
- 不采用 `tokio::Semaphore`：固定 Permit 数难以动态缩容，并会绕过未来统一 QueueTicket、epoch、超时和取消语义。
...
- 在 SettingRegistry 和 QueueTicket 完成前，辅助上限只能使用编译默认值或 Composition Root 注入值，满载行为固定为立即拒绝。
