...
- Runtime 每次只从解码器取得、编码并排队一个完整事件。协议桥合法消费心跳、注释或其他不产生下游事件的帧时，提交前循环必须继续排空解码器和当前 upstream chunk 中已经缓冲的完整帧，直到产生首个可接受事件、需要更多网络字节或确定 EOF；首事件完成后立即返回下游响应，不在提交前批量处理其后的公开事件。
...
- Runtime 测试验证原始/编码后字节耗尽、deadline、单事件预缓冲、桥接过滤帧与有效首事件位于同 chunk 时不等待后续网络、同 chunk 先交付后报错、会话绑定超时、提交后停止计费、健康归因、Guard 结算与错误边界。
