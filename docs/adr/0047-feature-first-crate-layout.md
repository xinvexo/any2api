...
功能相关文件可以从一个目录完整发现，测试与实现保持邻近，crate 根目录只呈现一级领域地图。应用装配、日志、停机和架构工具检查也具有明确所有权。迁移会一次性修改内部模块路径，但不改变跨 crate 稳定 API 或运行行为。新增文件必须先选择所属 feature；找不到所属边界通常意味着职责仍未定义清楚，而不是应当放回根目录。
