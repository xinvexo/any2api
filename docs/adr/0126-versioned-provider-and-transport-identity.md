...
4. Codex data 与 `/backend-api/wham` quota 保留显式不同的子 profile。quota 的 `Codex Desktop`/beta/language/fetch/priority 字段属于已有内部 Endpoint 契约，本决策不在没有新 capture 时猜测删改。差异必须在同一 identity 模块中可见，不再隐藏于 quota 解析器。
...
- Transport profile snapshot 断言 ID、policy versions、ALPN、HTTP 能力、keepalive、redirect 和 retry；现有 loopback TLS/H2 测试继续证明 isolation 域与物理连接行为。
