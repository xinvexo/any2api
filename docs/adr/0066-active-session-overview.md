...
- Web 标签固定使用“活动显式会话”和“建立中显式会话”。API 在关闭时仍返回两个零值，
  但页面展示“已关闭”与不可用状态，不能让用户误以为这是两个始终为零的请求计数器。
- `creating_session_count` 是首次显式绑定从取得 Lease 到提交或释放之间的瞬时 Gauge，
  不是累计建立次数。页面必须说明它通常会很快归零，禁止通过伪造累计值让数字看似活跃。

## 后果

会话粘性关闭时，管理 API 与当前生效策略一致地返回 `0`，Web 则直接显示关闭状态；两者都不会
为了视觉数字破坏 `previous_response_id` 的续接正确性。开启时，多轮 Responses 产生的多个
Response ID 不再被错计为多个会话，建立中的短暂零值也不会被误解为累计计数失效。
