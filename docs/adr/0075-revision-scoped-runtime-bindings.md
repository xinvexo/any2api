...
- Credential 的配置级可路由性由 PublishedSnapshot 候选决定，不在共享 Handle 上维护可重新激活旧 Binding 的
  全局 `retired` 开关。ADR-0167 另外增加只允许负向撤销的版本化 RouteAdmission：它不修改旧 Binding 的
  认证、RPM 或健康，但会阻止尚未开始的旧 Attempt 取得 StartPermit。
...
- 删除或轮换 Credential 不再反向改变已经取得 AttemptStartPermit 的请求认证或绑定提交结果；尚未取得 Permit
  的旧候选受 ADR-0167 的实时撤销约束。
