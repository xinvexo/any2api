...
- 状态：Superseded by ADR-0037 and ADR-0038
- 日期：2026-07-22
- 决策者：maintainer

> 取代说明（2026-07-25）：只读检查、健康分层和轻量计数继续有效；ADR-0037 将容量/负载率、
> 辅助占用和两类选择计数替换为可选 RPM、滚动窗口、剩余/到期时间、统一选择与 RPM 过滤计数。
> ADR-0038 进一步取消管理 API 和 Web 的逐账号运行态目录，只保留全局与 Provider 级聚合；
> Credential 级计数仍可作为进程内调度实现与测试细节存在。
